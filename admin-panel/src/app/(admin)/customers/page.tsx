            ...
            <tbody>
              {users.length === 0 ? (
                ...
                <tr key={u.id} className="border-b border-line last:border-b-0 row-hover">
                  ...
                  <td className="px-5 py-3"><StatusPill value={u.role} /></td>
                  <td className="px-5 py-3 text-muted tabular-nums">{new Date(u.createdAt).toLocaleDateString(undefined, { month: "short", day: "numeric", year: "numeric" })}</td>
                </tr>
              ))}
            </tbody>
          </table>
