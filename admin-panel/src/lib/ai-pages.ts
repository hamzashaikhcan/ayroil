...
const INGREDIENT_FACTS = `KEY INGREDIENTS (real, use these, never invent others; each has a purposeful role, never a random list): Rosemary oil (scalp care and stronger-looking hair support), Black seed oil (nourishment and traditional scalp care), Amla (hair strength, shine, and traditional hair care), Sweet almond oil (softness, nourishment, and dry hair care), Argan oil (smoother, shinier, healthier-looking hair), Vitamin E (nourishment and protection from dryness), Reetha and Shikakai (gentle, time-tested Ayurvedic cleansing and strengthening care), Hyaluronic acid (scalp hydration support), Castor oil (traditional root and thickness care). Describe each with "traditionally used for" or "known for" framing; never claim any ingredient cures dandruff, infection, or hair loss.`;
...
const PAGE_SPECS: Record<Exclude<AiPageKind, "faqs">, PageSpec> = {
  ...
  benefits: {
    ...
    sections: [
      ...
      "The key ingredients: ALL TEN from the ingredient facts, each in strong with 1-2 sentences on its purposeful role, using traditionally-known-for framing.",
