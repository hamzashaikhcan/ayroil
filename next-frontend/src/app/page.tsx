...
export default async function HomePage() {
	const [product, settings, allProducts, reviews] = await Promise.all([
		fetchPrimaryProduct().then((p) => p ?? FALLBACK_PRODUCT),
		fetchSettings(),
		fetchAllProducts(),
		fetchReviews(),
	]);
	const homeReviews = reviews.slice(0, 12);
				...
				<Container>
					<div className='mx-auto max-w-2xl text-center'>
						...
						<p className='mt-5 text-base leading-relaxed text-muted'>
							...
							quotes here come straight from verified orders.
						</p>
					</div>
					{homeReviews.length ? (
						<>
								...
								{homeReviews.map((r) => (
									<figure
											...
											{Array.from({ length: 5 }).map((_, i) => (
												<svg
													...
													className={`size-4 fill-current ${
														i < r.rating ? 'text-accent-deep' : 'text-line'
													}`}>
										...
										<blockquote className='mt-4 text-sm leading-relaxed text-ink'>
											&ldquo;{r.comment.trim() || `Rated ${r.rating} out of 5.`}&rdquo;
										</blockquote>
										<figcaption className='mt-5 flex items-center gap-3'>
											...
											<span className='leading-tight'>
												...
												<span className='block text-xs text-muted'>{formatDate(r.createdAt)}</span>
											</span>
										</figcaption>
									</figure>
								))}
